...
const input_stdin_array = ['5','5 5','2 7','8 10','10 20','4 5'];
const [numberOfInput, ...arr] = input_stdin_array;
for(let i = 0;i < numberOfInput;i++){
    let numbers = arr[i].split(" ");
    let outputs = [];
    for(let j = Math.min(numbers[0]); j <= Math.max(numbers[1]); j++){
        if(isPrime(j)){
            outputs.push(j);
        }
    }
    let output = outputs.length ? Math.max(...outputs) - Math.min(...outputs) : -1;
